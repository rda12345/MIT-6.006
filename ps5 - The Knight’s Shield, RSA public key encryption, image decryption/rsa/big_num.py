...
####################### Solution #######################
  def slow_mul(self, other):
    ...
    The method implements the standard multipication scheme.
    '''
    size_s = len(self.d)
    size_o = len(other.d)
    C = BigNum.zero(size_s + size_o)
    
    for i in range(size_s):
        carry = Byte.zero()
        for j in range(size_o):
            digit = self.d[i]*other.d[j] + C.d[i+j].word() + carry.word()
            C.d[i+j] = digit.lsb()
  ...
  def slow_divmod(self, other):
    ...
    '''
    # Initialization step, creates a list [d, d * 2, d *4, ...,d* 2**k,...,<=self]
    divisors = [BigNum(other.d)]
    remainder = BigNum(self.d)
    quotient = BigNum.zero()
    while divisors[-1] < remainder:
        divisors.append((divisors[-1]+divisors[-1]).normalize())     # the last element of divisors is > self
       
    for i in range(len(divisors)-1,-1,-1):
        quotient = quotient + quotient
        if divisors[i] <= remainder:
            quotient += BigNum.one()
