...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        #initializing nodes' queue keys and parents
        for node in nodes:
            node.queue_key = None
            node.parent = None
            node.visited = False
        ...
        # The NodeDistancePair is used as the priority queue key
        source.queue_key = NodeDistancePair(source, 0)
        frontier.insert(source.queue_key)
        num_visited = 0         # number of nodes visited
        
        while len(frontier) > 0:
            node_key = frontier.extract_min()
            node, dist = node_key.node , node_key.distance
            num_visited += 1
            node.visited = True
            if node == destination:
                break
            #relaxing the neighbors of node
            for neigh in node.adj:
                # relax edge (node,neigh)  
                neigh_distance = dist + weight(node,neigh)
                if neigh.queue_key == None:
                    neigh.queue_key = NodeDistancePair(neigh,neigh_distance)
                    frontier.insert(neigh.queue_key)
                    neigh.parent = node
                elif neigh.queue_key.distance > neigh_distance:
                    neigh.queue_key.distance = neigh_distance
                    neigh.parent = node
                    # Modifing the priority queue if needed
                    frontier.decrease_key(neigh.queue_key)
                    
        shortest_path = [destination]
        node = destination 
        while node != source:
            shortest_path.append(node.parent)
            node = node.parent
        shortest_path.reverse()
        return (shortest_path,num_visited)        
